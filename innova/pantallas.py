...
class PantallaMenu(ctk.CTkFrame):
    def __init__(
        ...
        *,
        ...
        ctk.CTkLabel(
            pie,
            text="Esc o Q para salir    ·    Fase 2b: menú, captura dinámica y DTW",
            font=ctk.CTkFont(size=12),
            text_color=tema.COLOR_TEXTO_MUDO,
        ).pack(side="left")
...
class PantallaCaptura(_PantallaConCamara):
    ...
    def _al_iniciar_hold(self) -> None:
        ...
        if not etiqueta:
            self._hold_activo = False
            self._avisar("Escribe la letra antes de grabar la trayectoria.", tema.COLOR_ERROR)
            return
    ...
    def _guardar_estatica(self) -> None:
        etiqueta = (self.ent_etiqueta.get() or "").strip()
        if not etiqueta:
            self._avisar("Escribe la letra o palabra de la seña antes de guardar.", tema.COLOR_ERROR)
            return
        if self._pipeline is None:
            self._avisar("No hay cámara ni modo demostración activo.", tema.COLOR_ERROR)
            return
...
class PantallaBiblioteca(ctk.CTkFrame):
    ...
    def _fila(self, item) -> None:
        ...
        ctk.CTkLabel(info, text=tipo, text_color=tema.COLOR_TEXTO, font=ctk.CTkFont(size=14)).pack(anchor="w")
